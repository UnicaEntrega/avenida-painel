		...
		<transition mode="out-in" enter-active-class="animated fadeIn" leave-active-class="animated fadeOut">
			<q-table :data="coletas" :columns="coletaColumns" align="left" row-key="id" :pagination.sync="pagination" @update:pagination="v=>buscar()" :loading="loading" @request="buscar" :rows-per-page-options="[10,20,50,100]" :pagination-label="paginationLabel" binary-state-sort>
				...
				<template v-slot:bottom>
					...
					<div class="col-12 text-right text-bold">Total entregas: R$ {{parseFloat(resumo.v).toFixed(2).replace('.',',')}}</div>
					<div class="col-12 text-right text-bold">Total comissão: R$ {{parseFloat(resumo.c).toFixed(2).replace('.',',')}}</div>
				</template>
			</q-table>
		</transition>
	</q-page>
	...
	data () {
		return {
			...
			coletaColumns: [
				...
				{ name: "comissao", label: "Comissão", field: "comissao", align: "left", format:val=>(val ? parseFloat(val).toFixed(2).replace('.',',') : '')+'%' },
				{ name: "valor_entrega", label: "Valor", field: "valor_entrega", align: "left", format:val=>'R$ '+(val ? parseFloat(val).toFixed(2).replace('.',',') : '') }
			],
			resumo: {q:0,v:0,c:0},
      loading: false,
      pagination: {
